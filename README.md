...
- **モバイルオーダー番号**として固定表示: **`A-102`**（デモ用。サーバ連携・採番ロジックは未実装）
- 受け取り案内の説明文
...
以下は**現リポジトリには未実装**です。会話で整理した店舗側（お店側）要件のメモです。

- 店舗ID・パスワード必須のログイン
- 店舗向けダッシュボード
- レジ機能（会計・注文確定と番号連携など）
- 待ち人数の増減（店舗側操作・来場者表示との同期）
- レストラン受付番号の表示・呼び出し進行（店舗側ディスプレイ）

バックエンドAPI、認証トークン、永続ストレージ、WebSocket 等は含まれていません。現状はすべてクライアント内のモックデータとローカル状態です。
